...
capol <- read.csv("data/province2.csv", sep =";")
capol <- capol[,-c(2)]
for(i in 1:length(capol$Longitudine)){
  capol$Longitudine[i] <- as.double(substring(as.character(capol$Longitudine[i]), 1, 5))
  capol$Latitudine[i] <- as.double(substring(as.character(capol$Latitudine[i]), 1, 5))
}

capol_reg <- capol

capol_reg[capol == 0.0] = NA
capol_reg=na.omit(capol_reg)
capol_reg <- capol_reg[, -c(4)]
rownames(capol_reg)  = c(1:length(capol_reg$Capoluogo))
